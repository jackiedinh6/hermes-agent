...
KANBAN_GUIDANCE = (
    ...
    "- **Attachments.** Attach real downloadable artifacts instead of pasting links in comments. For a file you "
    "wrote, use `kanban_attach_path` (Hermes reads the bytes off disk) — never hand-encode base64 and never "
    "publish an internal file to a paste site just to get a URL. `kanban_attach` (base64) is for bytes you are "
    "already holding; `kanban_attach_url` for a link you were given. 25 MB cap, `kanban_attachments` lists them. "
