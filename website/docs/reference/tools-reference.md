...
| `kanban_attach_path` | Attach a file that already exists on disk — Hermes reads the bytes itself (capped at 25 MB). The preferred way to attach an artifact you just wrote; the path is resolved the way `read_file` resolves one and must stay inside the task workspace. | `HERMES_KANBAN_TASK` or `kanban` toolset |
